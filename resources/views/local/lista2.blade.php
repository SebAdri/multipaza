				...
				<tr>
					...
					<td>
		                ...
		                {{-- <form style="display: inline" method="POST" action="{{ route('locales.destroy', $local->id) }}">
							...
							<button type="button" class="btn btn-danger" data-toggle="modal" data-target="#modalEliminar" id="buttonEliminar" value="{{$local->id}}"><i class="fa fa-trash"></i>  Eliminar</button>
...
<script type="text/javascript">
  ...
  $('#buttonEliminar').click(function(){
  	...
  	if (typeof $local !== "undefined" ) {
  		var id = $(this).val();
   		$('#formEliminar').attr('action', '{{ route('locales.destroy', $local->id) }}');
  });
